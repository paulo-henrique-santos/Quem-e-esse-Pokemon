...
def medio():
    ...
    global imagem
    if request.method == 'GET':
        aleatorio = random.randint(0, 251)
        API_ENDPOINT = f"https://pokeapi.co/api/v2/pokemon/{aleatorio}"
        response = requests.get(API_ENDPOINT)
        if response.status_code == 200:
            data = response.json()
            nome = data["forms"][0]["name"]
            imagem = data["sprites"]["other"]["official-artwork"]["front_default"]
            return render_template("medio.html", nome=nome, imagem=imagem)
    ...
    if request.method == "POST":
        ...
        if resposta:  # Só faz a verificação se algo foi digitado
            ...
            return render_template("medio.html", nome=nome, imagem=imagem, msg=msg)
...
def dificil():
    ...
    global imagem
    if request.method == 'GET':
        aleatorio = random.randint(0, 1025)
        API_ENDPOINT = f"https://pokeapi.co/api/v2/pokemon/{aleatorio}"
        response = requests.get(API_ENDPOINT)
        if response.status_code == 200:
            data = response.json()
            nome = data["forms"][0]["name"]
            imagem = data["sprites"]["other"]["official-artwork"]["front_default"]
            return render_template("dificil.html", nome=nome, imagem=imagem)
    ...
    if request.method == "POST":
        ...
        if resposta:  # Só faz a verificação se algo foi digitado
            ...
            return render_template("dificil.html", nome=nome, imagem=imagem, msg=msg)
